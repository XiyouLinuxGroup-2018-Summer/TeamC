...
int cnt=0;
void Quicksort(int cnt,int start);
...
void Quicksort(int cnt,int start)
{
    ...
    if(start >cnt)
        return ;
    strcpy(name,filenames[start]);
    i=start;
    j=cnt;
    while(i!=j)
    {
        while(strcmp(filenames[j],name)<=0 && i<j)
            j--;
        while(strcmp(filenames[i],name)>=0 && i<j)
            i++;
        if(i<j)
        {
                strcpy(name1,filenames[i]);
           strcpy(filenames[i],filenames[j]);
            strcpy(filenames[j],name1);
      }
    }
    strcpy(filenames[start],filenames[i]);
...
void display_single(int cnt,int alfag)
{
  ...
  Quicksort(cnt,0);
    ...
    {  int wordLenMax[4000]={0};//窗口的长度
     ...
     {
         int f=j*wordRowNum;
         for(i=0;i<wordRowNum&&f<terminalWidth;i++)//求出行数最宽的是
      {
          if(f>n1)
              break;
          if((int)strlen(filenames[f])>wordLenMax[j])
              wordLenMax[j]=(int)strlen(filenames[f]);
          f++;
      }
         if(j!=cnt-1)
         sum+=wordLenMax[j]+2;

...
void* thread(void * sockfd)
{
    ...
    char name[32], recvnbuf[1024], sendnbuf[1024], sendntime[32];
    len = recvn(sock_fd, name, 32, 0);
    if (len < 0)
    {
    ...
    len = sendn(sock_fd, "welcome to join group\n", 22, 0);
    if (len < 0)
    {
        fprintf(stderr, "%d: ", __LINE__);
        perror("sendn");
    ...
    while(1)
    {
        memset(sendnbuf, '\0', sizeof(sendnbuf));
        memset(recvnbuf, '\0', sizeof(recvnbuf));
        len = recvn(sock_fd, recvnbuf, 1024, 0);
        if (len < 0)
        {
            fprintf(stderr, "%d: ", __LINE__);
            perror("recvn");
        ...
        if (len == 0)
        {
            sprintf(sendnbuf, "[System Message] %s quit group\n", name);
            flag = 0;
        }
        get_time(sendntime);
        if (flag)
        {
            sprintf(sendnbuf, "%s[%s] \"%s\"", sendntime, name, recvnbuf);
        }
        for (int i = 0; i < 51; i++)
        {
            if (sockfds[i] == 0)
            ...
            pthread_mutex_lock(&mutex);
            len = sendn(sockfds[i], sendnbuf, 1024, 0);
            pthread_mutex_unlock(&mutex);
            if (len < 0)
            {
                fprintf(stderr, "%d: ", __LINE__);
...
int main(void)
{
    ...
    char recvnbuf[1024], sendnbuf[1024]; 

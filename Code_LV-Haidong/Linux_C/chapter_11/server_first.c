...
void add_offline_news(qq information, char *go, char *come)
{
    ...
    char query1[200];
    sprintf(query1, "insert into off_line_record_%s (go,come,record) values('%s','%s','%s');", information.to, go, come, information.record);
...
void init_file(qq information, int conn_fd)
{
    ...
    sprintf(query2, "insert into chat_record(my_number,friend_number,record) values('%s','%s', 'send file :%s');", information.number, information.to, information.passwd); //加入私聊记录
    t = mysql_real_query(conn, query1, strlen(query1));
    if (t != 0)
    {
        printf("%s\n", mysql_error(conn));
...
void send_file(qq information, int conn_fd) //转发文件
{
    ...
    {
        if ((fd = search(head, information.to))) //
            information.state = -6;
        send(fd, &information, sizeof(qq), 0);
    }
    else if ((fd = search(head, information.to))) //
    {
        information.state = -6;
        send(fd, &information, sizeof(qq), 0);
    }
...
//    printf("sssssssssssssssssssss-------\n");
     while ((row = mysql_fetch_row(result)))
     {
      ...
      for(t = 0 ;t<mysql_num_fields(result);t++)
       {
        sprintf(query2, "update friends_%s set friends_state = 0  where friends_number='%s' ; ", row[t], information.number);
        printf("query2 : %s \n", query2);
...
void logout(qq information,int conn_fd)
{
    char query[200];
    MYSQL_RES * my_result;
    sprintf(query, "update friends_%s set friends_state = 0  where friends_number='%s' ; ", information.number,information.number);
    int y =mysql_real_query(conn,query,strlen(query));
    if (y != 0)
    {
        printf("%s \n", mysql_error(conn));
    }
...
void Stop(int signo)
{
    ...
    while (temp != NULL)
    {
        sprintf(query1, "select friends_number from friends_%s;",temp->QQ);
        int t, y = 0;
        t = mysql_real_query(conn, query1, strlen(query1));
        //获取函数的结果集
        printf("ssss t %d\n ",t);
        ...
        for(t = 0 ;t<mysql_num_fields(result);t++)
        {
            sprintf(query2, "update friends_%s set friends_state = 0  where friends_number='%s' ; ", row[t], temp->QQ);
            printf("query2 : %s \n", query2);
...
int intput(int conn_fd)
{
    // int conn_fd = (int)arg;
    qq information;
    int t;

    t = recvn(conn_fd, &information, sizeof(qq), 0);
    signal(SIGINT,Stop);
    printf("t %d\n", t);
    if (t > 0)
    {
        strcpy(login_number, information.number);
    }
    //printf("conn_fd %d \n",conn_fd);
    //在这里根据recv的返回值来进行登陆状态的改变
    else if (t == 0)
    {
        strcpy(information.number,login_number);
      
        if(b <= 7)
        {
            close(conn_fd);
            off_onlie(information);//更新好友列表,错误退出  
    ...
    switch (information.flag)
    {
        ...
        case 10:
            ...
            break;
        case 11:
            group_member_list(information, conn_fd);
            ; //进入群聊//包括设置管理员权限和一对多聊天//邀请人,踢人
            break;
        case 12:
...
void look_offline_news(qq information, int conn_fd)
{
    ...
    sprintf(query1, "select * from off_line_record_%s ; ", information.number);
    sprintf(query2, "delete from off_line_record_%s ; ", information.number);
    ...
    while (row = mysql_fetch_row(result))
    {
        ...
        while (y != mysql_num_fields(result))
        {
            if (y == 0)
            {
                strcpy(server_information.to, row[y]);
                printf("row 0 %s \n", row[y]);
                y++;
            }
            if (y == 1)
            {
                strcpy(server_information.number, row[y]);
                printf("row 1 %s \n", row[y]);
                y++;
            }
            if (y == 2)
            {
                strcpy(server_information.record, row[y]);
                printf("row 1 %s \n", row[y]);
                y++;
            }
            y++;
...
void group_list(qq information, int conn_fd)
{
    ...
    while (row = mysql_fetch_row(result))
    {
        ...
        while (y != mysql_num_fields(result))
        {
            if(strcmp(information2.record,row[y]) == 0)
            {
                y++;
                continue;
            }
...
void chat_black(qq information, int conn_fd)
{
    ...
    memset(query1,0,sizeof(qq));
    sprintf(query1,"update friends_%s set friends_state = 2  where friends_number= '%s';",information.to,information.number);
    t = mysql_real_query(conn,query1,strlen(query1));
    if(t != 0)
    {
        printf("%s\n",mysql_error(conn));
...
void regist(qq information, int conn_fd) //每个用户都需要创建一个自己的表,同时操作应该保留在日志中
{
    ...
    char *mysql_query1, *mysql_query2, *mysql_query3, *mysql_query4,*mysql_query5;
    ...
    else
    {
        mysql_free_result(result);
        // printf("number %s\n",information.number);
        sprintf(mysql_query2, "create table friends_%s (`friends_id` INT UNSIGNED AUTO_INCREMENT,`friends_number`VARCHAR(100),`friends_name` VARCHAR(100),`friends_state` VARCHAR(100), unique index(`friends_number`),PRIMARY KEY ( `friends_id` ))ENGINE=InnoDB DEFAULT CHARSET=utf8;", information.number);
        ...
        sprintf(mysql_query5,"insert into friends_%s (friends_number,friends_name,friends_state)  values(%s,%s,1);", information.number,information.number,information.username);
        t = mysql_real_query(conn, mysql_query2, strlen(mysql_query2));
        result = mysql_store_result(conn); //API并不会主要释放句柄中的内容
        mysql_free_result(result);
        if (t != 0)
...
void login(qq information, int conn_fd)
{
    ...
    char *mysql_login, *mysql_passwd, *mysql_log,*mysql_denglu;
    int temp;
    MYSQL_RES *result; //mysql的结果集
    MYSQL_ROW row;
    ...
    mysql_free_result(result);
     ...
     mysql_denglu = (char *)malloc(300);
     memset(mysql_denglu,0,300);
     sprintf(mysql_denglu,"select   friends_state from  friends_%s  where friends_number='%s' and friends_state = 1; ", information.number,information.number);
     temp = mysql_real_query(conn,mysql_denglu,sizeof(qq));
     printf("mysql_denglu %s \n",mysql_denglu);
     result = mysql_store_result(conn);
     row = mysql_fetch_row(result);
     if (row != NULL)
     {
    ...
    else
    {
        ...
        sprintf(mysql_passwd, "select users_number,users_passwd from users where users_number='%s' and users_passwd=md5('%s');", information.number, information.passwd);
        //  printf("mysql_passwd %s\n", mysql_passwd);
        ...
        if (result != NULL)
        {
            ...
            memset(mysql_denglu,0,300);
            mysql_free_result(result);
            sprintf(mysql_denglu,"update friends_%s set friends_state =1 where friends_number ='%s' ; ",information.number,information.number);
           
            head = add(head, information, conn_fd);


            temp = mysql_real_query(conn,mysql_denglu,strlen(mysql_denglu));
            printf("deng %s\n",mysql_denglu);
            if(temp != 0)
            {
                printf("%s\n",mysql_error(conn));
    ...
    while (1)
    {
        ...
        for (i = 0; i < nfds; ++i)
        {
            ...
            if (events[i].data.fd == sock_fd)
            {
                ...
                printf("accpet ip from %s\n", inet_ntoa(cli_addr.sin_addr)); //取得客户端的ip

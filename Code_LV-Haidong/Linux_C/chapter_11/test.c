...
void* thread(void * sockfd)
{
    ...
    while (1)
    {
        len = recvn(sock_fd, recvnbuf, 1023, 0);
        if (len < 0)
            my_err("recvn", __LINE__);
        printf("%s\n", recvnbuf);
    }
}
...
int main(void)
{
    ...
    // sendt [username] - message
    ret = sendt(sock_fd, tempname, strlen(tempname), 0);
    if (ret < 0)
        my_err("sendt", __LINE__);
    
    // recvn [welcome message]
    ret = recvn(sock_fd, recvnbuf, 1023, 0);
    if (ret < 0)
        my_err("recvn", __LINE__);
    printf("Server: %s\n", recvnbuf);
    ...
    while (1)
    {
        ...
        if (tempstring[0] == '\n')
            continue;
        printf("\n");
        ret = sendt(sock_fd, tempstring, strlen(tempstring), 0);
        if (ret < 0)
            my_err("sendt", __LINE__);
        sleep(1);
    }

...
void recvfile(qq information,int conn_fd)
{

    char file_name[100];
    int fd  ;
    int g;
    sscanf(information.passwd,"%d",&g);//以特定的格式读取
    sprintf(file_name,"/mnt/%s_%s",information.to,information.number);
    fd = open(file_name,O_RDWR | O_APPEND | O_CREAT ,S_IRUSR | S_IWUSR); 
    if( fd < 0 )
        my_err("此文件无法打开 ",__LINE__);
    write(fd,information.record,g);
...
void send_file(qq information2, int conn_fd)
{
    ...
    for(i=0,j=0;j<strlen(name);i++)
    {
        ...
        information2.record[j++] = name[i];
    }
    information2.record[j]='\0';
    ...
    while(sum != sum_length)
    {
       ...
       if(file_len == -1)
       {
        ...
        sendn(my_connfd,&information2,sizeof(qq),0);
    }
    printf("文件传输结束 < < <  <\n");
...
int my_recv(void)
{
    while (1)
    {
        ...
        int t;
        t = recv(my_connfd, &information2, sizeof(qq), 0);
        // if (t == 0)
        // {
        ...
        switch (information2.flag)
        {
            ...
            {   
                ...
                {
                         recvfile(information2,my_connfd);
                }
                else
                {
...
void view_myfriend(qq information, int conn_fd)
{
    ...
    while (1)
    {
        printf("pleases chatting_with your_friend\n");
        printf("to %s\n",to);
        scanf("%s[^\n]",information.record);
...
void  get_friend_news(qq information)
{
     ...
     if(strcmp(to,information.number) == 0)
     {
         printf("好友%s:  %s\n",information.to,information.record);
     }
     else

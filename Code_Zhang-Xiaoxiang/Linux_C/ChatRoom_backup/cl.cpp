    ...
    while (1)
    {
        ...
        switch (kind)
        {
            ...
            case Flag_Cmd_RecvFile: // 接收文件
            {
                int fd, ret; 
                char filename[256];
                sprintf(filename, "./%d", recvpack.source_id);
                ...
                {
                    fd = open(filename, O_RDWR | O_APPEND);     // 追加
                    if (fd == -1)
                    {
                        perror("open");
                        break;
                    }
                    write(fd, recvpack.strmsg, (size_t)recvpack.statusflag);
                    printf(">> 接收到一个包\n");
                    close(fd);
                }

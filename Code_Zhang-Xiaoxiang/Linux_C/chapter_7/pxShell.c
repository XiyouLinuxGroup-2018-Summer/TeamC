    ...
    for (int i = 0, j = 0; i <= flag; i++)
    {
        ...
        for (int t = 0; (success != -1) && file_for_output[i][t] != NULL; t++)
        {
            ...
            if (redirect_flag[i][t] == 2)    // >>
                fd_temp = open(file_for_output[i][t], O_RDWR | O_CREAT | O_APPEND, 0644);
            else if (redirect_flag[i][t] == 1)
                fd_temp = open(file_for_output[i][t], O_RDWR | O_CREAT | O_TRUNC, 0644);
            // printf("fd_temp = %d, fout = %d\n", fd_temp, fout);
            // CopyFileC(fd_temp, fout);
            char buf[1024];
            int len;
            while((len = read(fout, buf, 1024)) != 0)
            {
                printf("fd->new = %d\nbuf = %s\n", fout, buf);
                write(fd_temp, buf, len);
            }
            close(fd_temp);
        }
        close(fout);                            // 关闭fd
